...
function Flashcards() {
  ...
  useEffect(() => {
    // Get the deck ID from the URL or use the default 'flashcards' key
    const storageKey = deckId || "flashcards";
    ...
    if (stored) {
      ...
      // If no deck found, redirect to the decks page
      navigate("/decks");
    }
  }, [deckId, navigate]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-blue-100 p-2 sm:p-4 md:p-8 flex flex-col">
      <div className="max-w-4xl mx-auto w-full flex-grow flex flex-col items-center justify-center pb-16">
        <h1 className="text-2xl sm:text-3xl font-bold text-indigo-900 mb-6 text-center">
          {deckName}
        </h1>
